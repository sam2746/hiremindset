...
기대 응답:
    {
        "action": "accept" | "fallback" | "drill" | "pass" | "inject",
...
- drill  : 답변은 괜찮지만 더 깊게 파고들 여지가 있음. seed_drill_probe가 답변을
           ...
           control=drill
- pass   : 답변이 의미 없거나 추궁 가치 없음. flag도 건드리지 않고 그냥 다음 큐로.
           control=continue
...
def apply_decision_response(
    ...
    if action not in {"accept", "fallback", "drill", "pass", "inject"}:
